...
protected void configure(AuthenticationManagerBuilder auth) throws Exception{
	auth.inMemoryAuthentication().withUser("Sourav").password("Password").roles("USER");
}	
...
protected void configure(HttpSecurity http) throws Exception{
	...
	// here we are fully implementing the security for every request so we use fully authenticated
	http.authorizeHttpRequests().antMatchers("/user").hasRole("USER").antMatchers("/").permitAll().and().formLogin();
